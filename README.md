...
работает как история выданных треков.
...
**Своя страница**: запросы треков, сохранения, подписчики. Видит только он.
...
**Чего слушатель не видит:** публичных счётчиков запросов, лайков и подписчиков —
...
**Приватность.** В настройках — «Стереть мою историю»: удаляет историю запросов,
...
| Публичные счётчики запросов, лайков, подписчиков | Слушатель не видит ни одного. Артист видит только свои запросы и сохранения. |
...
- **Обложка живёт на релизе, а thumbnail — у каждого аудио отдельно.** Трек
  берёт обложку релиза для карточки, если у него нет своей. Если Telegram
  прислал аудио без thumbnail, после появления обложки бот один раз пересоздаёт
  аудио с компактной копией картинки и сохраняет новый `file_id`.
...
Оригинальные аудиобайты не хранятся локально: Telegram остаётся источником
медиа. Уже загруженный `file_id` нельзя изменить, поэтому для голого аудио
создаётся новый Telegram-файл с thumbnail; при ошибке старый `file_id`
сохраняется рабочим.
...
Три сигнала, смешиваются по тому, что человек реально запросил или сохранил, потом
диверсифицируются.
...
запросов и сохранений с подавлением популярности:
...
остаётся читаемым логом выданных треков, а не стеной меню.
